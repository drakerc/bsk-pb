...
class Des:
    ...
    def bit_array_to_string(self, array):  # Recreate the string from the bit array
        res = ''.join([chr(int(y, 2)) for y in [''.join([str(x) for x in bytes]) for bytes in self.nsplit(array, 8)]])
        return res
    ...
    def string_to_bit_array(self, text):  # Convert a string into a list of bits
        array = list()
        for char in text:
    ...
    def substitute(self, d_e):  # Substitute bytes using SBOX
        subblocks = self.nsplit(d_e, 6)  # Split bit array into sublist of 6 bits
        result = list()
        for i in range(len(subblocks)):  # For all the sublists
            block = subblocks[i]
            row = int(str(block[0]) + str(block[5]), 2)  # Get the row with the first and last bit
            column = int(''.join([str(x) for x in block[1:][:-1]]), 2)  # Column is the 2,3,4,5th bits
            val = self.S_BOX[i][row][column]  # Take the value in the SBOX appropriated for the round (i)
            bin = self.binvalue(val, 4)  # Convert the value to binary
    ...
    def generate_keys(self):  # Algorithm that generates all the keys
        ...
        key = self.permut(key, self.CP_1)  # Apply the initial permut on the key
        g, d = self.nsplit(key, 28)  # Split it in to (g->LEFT),(d->RIGHT)
        for i in range(16):  # Apply the 16 rounds
            g, d = self.shift(g, d, self.SHIFT[i])  # Apply the shift associated with the round (not always 1)
            tmp = g + d  # Merge them
            self.keys.append(self.permut(tmp, self.CP_2))  # Apply the permut to get the Ki

    def shift(self, g, d, n):  # Shift a list of the given value
        return g[n:] + g[:n], d[n:] + d[:n]

    def add_padding(self):  # Add padding to the datas using PKCS5 spec.
        pad_len = 8 - (len(self.text) % 8)
        self.text += pad_len * chr(pad_len)

    def remove_padding(self, data):  # Remove the padding of the plain text (it assume there is padding)
        pad_len = ord(data[-1])
        return data[:-pad_len]
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        text_blocks = self.nsplit(self.text, 8)  # Split the text in blocks of 8 bytes so 64 bits
        result = list()
        for block in text_blocks:  # Loop over all the blocks of data
            block = self.string_to_bit_array(block)  # Convert the block in bit array
            block = self.permut(block, self.PI)  # Apply the initial permutation
            ...
            for i in range(16):  # Do the 16 rounds
                ...
                g = d
                d = tmp
            result += self.permut(d + g, self.PI_1)  # Do the last permut and append the result to result
