...
class CiphertextAutokey():
    def __init__(self):
        ...
        encodeB = Button(rootB, command=partial(self.start_cypher, 'encode'), text='Enkoduj')
        encodeB.grid(columnspan=2, sticky=E + W, padx=10, pady=10)

        encodeB = Button(rootB, command=partial(self.start_cypher, 'decode'), text='Dekoduj')
        encodeB.grid(columnspan=2, sticky=E + W, padx=10, pady=10)

        rootB.mainloop()

    def start_cypher(self, type):
        seed = seedText.get('1.0', 'end').rstrip()
        input = inputEntry.get('1.0', 'end').rstrip()
        output = outputEntry.get('1.0', 'end').rstrip()

        # key = "apfmgdg pdosfgpndf"
        # # text = "Moja mama ciagle biega najpierw praca pozniej domfffffffff"
        # f = open('p.png', "r")
        # a = np.fromfile(f, dtype=np.uint8)
        # output = open('p_out.png', 'wb')
        # print(a)
        # a_str = ''
        # for i in a:
        #     a_str += str(chr(i))
        # #print(a_str)
        # d = Des()
        # r = d.encrypt(key, a_str, True)
        # r2 = d.decrypt(key, r, True)
        #
        #
        # print("Ciphered: %r" % r)
        # print("Deciphered: ", r_decrypted)
        # output.write(bytearray(r_decrypted))
...
class Menu():
    def __init__(self):
        ...
        ciphertextAutokey = Button(rootC, command=partial(CiphertextAutokey), text='Algorytm DES', height=2)
...
key = "apfmgdg pdosfgpndf"
# text = "Moja mama ciagle biega najpierw praca pozniej domfffffffff"
f = open('sprawozdanie-bsk-1.pdf', "r")
a = np.fromfile(f, dtype=np.uint8)
output = open('sprawozdanie-bsk-1-out.pdf', 'wb')
print(a)
a_str = ''
for i in a:
    a_str += str(chr(i))
...
output.write(bytearray(out_arr))

print("Ciphered: %r" % r)
print("Deciphered: ", out_arr)
output.write(bytearray(out_arr))
